...
{
  public static final int XYZ_LENGTH = 3;
                                             ...
                                             double fractionFrom0)
  {
    ...
    double[] p = Vec.computePointOnLine (p0, p1, fractionFrom0);
  ...
  public static void extendLine (double[] p0, double[] p1, double toLength,
                                 double anchor)
  {
    double[] p = Vec.computePointOnLine (p0, p1, anchor);
    
    double length0 = toLength * anchor;
    double length1 = toLength * (1.0 - anchor);
    
    Vec.extendLine (p, p0, length0);
    Vec.extendLine (p, p1, length1);
  }
  ...
  public static void extendLine (double[] p0, double[] p1, double toLength)
  {
    double oldLength = Vec.length (p0, p1);
  ...
  public static float[] normalize(float[] xyz) {
    float length = Vec.length(xyz);
    if (length != 0) {
  ...
  public static double[] normalize(double[] xyz) {
    double invLength = 1.0 / Vec.length(xyz);
    xyz[0] *= invLength;
    xyz[1] *= invLength;
    xyz[2] *= invLength;
    return xyz;
  ...
  {
    double[] v = Vec.createVector (p0, p1);
    return length (v);
  }
  ...
  {
    return Vec.length ((double) x0, (double) y0,
                            (double) x1, (double) y1);
  }
  ...
  public static double length (int[] x, int[] y, boolean isClosed)
  {
    ...
    for (int i = 0; i < nPoints-1; i++)
      length += Vec.length (x[i], y[i], x[i+1], y[i+1]);

    // Add last leg if this is a polygon
    if (isClosed && nPoints > 1)
      length += Vec.length (x[nPoints-1], y[nPoints-1], x[0], y[0]);
                                 ...
                                 int x, int y)
  {
    ...
    double length = Vec.length (x0, y0, x1, y1);
    if (length == 0.0)
      return Vec.length (x0, y0, x, y);
  ...
  {
    double[] v0 = Vec.createVector (p0, p1);
    double[] v1 = Vec.createVector (p0, p2);

    double dotProduct = Vec.computeDotProduct (v0, v1);

    double length1 = Vec.length (v0);
    double length2 = Vec.length (v1);
  ...
  public static float computeDotProduct (float[] v0, float[] v1)
  {
    return v0[0] * v1[0] + v0[1] * v1[1] + v0[2] * v1[2];
  }
  ...
  public static float[] computeCrossProduct (float[] v0, float[] v1)
  {
    return computeCrossProduct(v0, v1, new float[3]);
  }
  public static float[] computeCrossProduct (float[] v0, float[] v1, float crossProduct[])
  {
    crossProduct[0] = v0[1] * v1[2] - v0[2] * v1[1];
    crossProduct[1] = v0[2] * v1[0] - v0[0] * v1[2];
    crossProduct[2] = v0[0] * v1[1] - v0[1] * v1[0];
  ...
  {
    int s1 = Vec.sameSide (x0, y0, x1, y1, x2, y2, x3, y3);
    int s2 = Vec.sameSide (x2, y2, x3, y3, x0, y0, x1, y1);
                                                     ...
                                                     int x1, int y1)
  {
    ...
    if (Vec.isPointInsideRectangle (x0, y0, x1, y1, lx0, ly0) ||
        Vec.isPointInsideRectangle (x0, y0, x1, y1, lx1, ly1))
      return true;

    // If it intersects it goes through. Need to check three sides only.

    // Check against top rectangle line
    if (Vec.isLineIntersectingLine (lx0, ly0, lx1, ly1,
                                         x0, y0, x1, y0))
      return true;
                                                         ...
                                                         int x1, int y1)
  {
    ...
    if (Vec.isPointInsideRectangle (x[0], y[0], x0, y0, x1, y1))
      return true;
    ...
    for (int i = 1; i < x.length; i++) {
      if (x[i-1] != x[i] || y[i-1] != y[i])
        if (Vec.isLineIntersectingRectangle (x[i-1], y[i-1],
                                                  x[i], y[i],
                                                  x0, y0, x1, y1))
          return true;
    }
                                                        ...
                                                        int x1, int y1)
  {
    ...
    if (n == 1)
      return Vec.isPointInsideRectangle (x0, y0, x1, y1, x[0], y[0]);
    
    //
    // If the polyline constituting the polygon intersects the rectangle
    // the polygon does too.
    //
    if (Vec.isPolylineIntersectingRectangle (x, y, x0, y0, x1, y1))
      return true;
  
    // Check last leg as well
    if (Vec.isLineIntersectingRectangle (x[n-2], y[n-2], x[n-1], y[n-1],
                                              x0, y0, x1, y1))
      return true;
    ...
    // or separate.
    //
    if (Vec.isPointInsidePolygon (x, y, x0, y0) ||
        Vec.isPointInsideRectangle (x0, y0, x1, y1, x[0], y[0]))
      return true;
  ...
  public static double[] computePolygonCentroid (double[] x, double[] y)
  {
    ...
    double area = Vec.computePolygonArea (x, y);
                                                 ...
                                                 double[] intersection)
  {
    ...
    double a0 = Vec.equals (x0, x1, LIMIT) ?
                INFINITY : (y0 - y1) / (x0 - x1);
    double a1 = Vec.equals (x2, x3, LIMIT) ?
    ...
    if (Vec.equals (a0, INFINITY)) {
      x = x0;
      y = a1 * x + b1;
    }
    else if (Vec.equals (a1, INFINITY)) {
      x = x2;
      y = a0 * x + b0;
    }
    else {
    ...
    if (Vec.equals (x0, x1)) {
      if (y0 < y1)
        distanceFrom1 = y < y0 ? Vec.length (x, y, x0, y0) :
                        y > y1 ? Vec.length (x, y, x1, y1) : 0.0;
      else
        distanceFrom1 = y < y1 ? Vec.length (x, y, x1, y1) :
                        y > y0 ? Vec.length (x, y, x0, y0) : 0.0;
    }
    else {
      if (x0 < x1)
        distanceFrom1 = x < x0 ? Vec.length (x, y, x0, y0) :
                        x > x1 ? Vec.length (x, y, x1, y1) : 0.0;
      else
        distanceFrom1 = x < x1 ? Vec.length (x, y, x1, y1) :
                        x > x0 ? Vec.length (x, y, x0, y0) : 0.0;
    }
    
    double distanceFrom2;
    if (Vec.equals (x2, x3)) {
      if (y2 < y3)
        distanceFrom2 = y < y2 ? Vec.length (x, y, x2, y2) :
                        y > y3 ? Vec.length (x, y, x3, y3) : 0.0;
      else
        distanceFrom2 = y < y3 ? Vec.length (x, y, x3, y3) :
                        y > y2 ? Vec.length (x, y, x2, y2) : 0.0;
    }
    else {
      if (x2 < x3)
        distanceFrom2 = x < x2 ? Vec.length (x, y, x2, y2) :
                        x > x3 ? Vec.length (x, y, x3, y3) : 0.0;
      else
        distanceFrom2 = x < x3 ? Vec.length (x, y, x3, y3) :
                        x > x2 ? Vec.length (x, y, x2, y2) : 0.0;
    }
    
    return Vec.equals (distanceFrom1, 0.0) &&
      Vec.equals (distanceFrom2, 0.0) ? 1 : 0;
  }
                                                       ...
                                                       double x1, double y1)
  {
    ...
    for (int i = 0; i < nPoints; i++) {
      ...
      if (Vec.equals (x2, x3) && Vec.equals (y2, y3))
        continue;

      int type = Vec.findLineSegmentIntersection (x0, y0, x1, y1,
                                                       x2, y2, x3, y3,
                                                       intersection);
      ...
      if (type == -2) { // Overlapping
        ...
        int side = Vec.sameSide (x0, y0, x1, y1,
                                      x[p1], y[p1], x[p2], y[p2]);
  ...
  public static int[] createEllipse (int[] x, int[] y)
  {
    ...
    double axisAngle = Vec.computeAngle (p0, p1, p2);
  
    // dx / dy  
    double dx = Vec.length (x0, y0, x[1], y[1]);
    double dy = Vec.length (x0, y0, x[0], y[0]) * Math.sin (axisAngle);
    ...
    // it to zero as well.
    if (!Vec.equals (axisAngle, Math.PI/2.0, 0.1) && 
        !Vec.equals (axisAngle, Math.PI,     0.1) &&
        !Vec.equals (axisAngle, 0.0,         0.1)) {
                                   ...
                                   double length, double angle, double inset)
  {
    ...
    int[] pos1 = new int[2];
    Vec.findPolygonPosition (x, y, length, pos1);
                                             ...
                                             int[] position)
  {
    ...
    for (int i = 1; i < x.length; i++) {
      double legLength = Vec.length (x[i-1], y[i-1], x[i], y[i]);
